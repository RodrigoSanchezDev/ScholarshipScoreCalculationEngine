...
------------------------------------------------------------
-- Evaluación Final Transversal - "Becas para docentes de la educación pública"
------------------------------------------------------------

------------------------------------------------------------
-- Función: f_formatea_rut
------------------------------------------------------------
CREATE OR REPLACE FUNCTION f_formatea_rut(p_rut IN NUMBER) RETURN VARCHAR2 IS
  v_rut_str VARCHAR2(20);
  v_body    VARCHAR2(20);
  v_sum     NUMBER := 0;
  v_mult    NUMBER := 2;
  v_digit   NUMBER;
  v_dv      VARCHAR2(1);
BEGIN
  -- Convertir el número a cadena y rellenarlo a 8 dígitos con ceros a la izquierda.
  v_rut_str := LPAD(TO_CHAR(p_rut), 8, '0');
  v_body := v_rut_str;
  
  -- Calcular el dígito verificador multiplicando cada dígito de derecha a izquierda por una secuencia que se reinicia en 2.
  FOR i IN REVERSE 1..LENGTH(v_body) LOOP
    v_digit := TO_NUMBER(SUBSTR(v_body, i, 1));
  ...
  -- Determinar el dígito verificador usando el módulo 11.
  v_dv := CASE (11 - MOD(v_sum, 11))
            WHEN 11 THEN '0'
            WHEN 10 THEN 'K'
            ELSE TO_CHAR(11 - MOD(v_sum, 11))
          END;
  
  -- Formatear y devolver el RUT.
  RETURN SUBSTR(v_rut_str, 1, 2) || '.' ||
         SUBSTR(v_rut_str, 3, 3) || '.' ||
         SUBSTR(v_rut_str, 6, 3) || '-' || v_dv;
EXCEPTION
  WHEN OTHERS THEN
    RETURN TO_CHAR(p_rut);
END f_formatea_rut;
/
------------------------------------------------------------
-- Procedimiento: prc_registra_error
-- Objetivo: Registrar errores en la tabla ERROR_PROCESO únicamente cuando la rutina sea 'f_calculo_experiencia'.
------------------------------------------------------------
CREATE OR REPLACE PROCEDURE prc_registra_error(
  p_numrun  IN NUMBER,
  p_rutina  IN VARCHAR2,
  p_mensaje IN VARCHAR2
) IS
  v_rutina VARCHAR2(100);
BEGIN
  -- Si el error proviene de f_calculo_experiencia, usamos "PA PRINCIPAL PROCESO"
  IF p_rutina = 'f_calculo_experiencia' THEN
    v_rutina := 'PA PRINCIPAL PROCESO';
  ELSE
    v_rutina := p_rutina;
  END IF;
  
  BEGIN
    UPDATE ERROR_PROCESO
       SET rutina_error = v_rutina,
           mensaje_error = SUBSTR(p_mensaje, 1, 250)
    WHERE numrun = p_numrun;
    
    IF SQL%ROWCOUNT = 0 THEN
      INSERT INTO ERROR_PROCESO(numrun, rutina_error, mensaje_error)
      VALUES (p_numrun, v_rutina, SUBSTR(p_mensaje, 1, 250));
    END IF;
  EXCEPTION
    WHEN OTHERS THEN
      NULL; -- Ignorar cualquier error durante el registro
  END;
END;
/
------------------------------------------------------------
-- PASO 0: Truncar las tablas de resultados y errores
------------------------------------------------------------
BEGIN
  EXECUTE IMMEDIATE 'TRUNCATE TABLE DETALLE_PUNTAJE_POSTULACION';
  EXECUTE IMMEDIATE 'TRUNCATE TABLE ERROR_PROCESO';
  EXECUTE IMMEDIATE 'TRUNCATE TABLE RESULTADO_PROCESO';
  COMMIT;
END;
/
------------------------------------------------------------
-- Package: pkg_extra
-- Objetivo: Calcular el puntaje extra (regla 1.3)
------------------------------------------------------------
CREATE OR REPLACE PACKAGE pkg_extra IS
  v_ptje_extra NUMBER;
  FUNCTION f_calculo_extra(
    p_id         IN NUMBER,  -- NUMRUN del postulante
    p_porcentaje IN NUMBER,  -- Porcentaje a aplicar (por ejemplo, 35)
    p_ptje_exp   IN NUMBER,  -- Puntaje por años de experiencia
    p_ptje_pais  IN NUMBER   -- Puntaje por país
  ) RETURN NUMBER;
END pkg_extra;
/

CREATE OR REPLACE PACKAGE BODY pkg_extra IS
  FUNCTION f_calculo_extra(
    p_id         IN NUMBER,
    ...
    v_extra       NUMBER := 0;
  BEGIN
    -- Calcula la cantidad de establecimientos y la suma de horas semanales para el postulante.
    SELECT COUNT(DISTINCT al.establecimiento),
           NVL(SUM(al.horas_semanales), 0)
      INTO v_cant_estab, v_total_horas
    FROM ANTECEDENTES_LABORALES al
    WHERE al.numrun = p_id;
    
    -- Si el postulante trabaja en más de 1 establecimiento y acumula más de 30 horas semanales, se calcula el puntaje extra.
    IF v_cant_estab > 1 AND v_total_horas > 30 THEN
      v_extra := ROUND((p_ptje_exp + p_ptje_pais) * (p_porcentaje/100));
    END IF;
    
    v_ptje_extra := v_extra;
    RETURN v_extra;
  EXCEPTION
    WHEN OTHERS THEN
      RETURN 0;
...
/
------------------------------------------------------------
-- PASO 2: Funciones Almacenadas (agrupadas en este caso en pkg_calculos podría ser una opción)
------------------------------------------------------------

/* Función: f_calculo_experiencia
   Objetivo: Calcular el puntaje basado en los años de experiencia del postulante,
             considerando una fecha de corte para simular el cálculo a una fecha fija.
*/
CREATE OR REPLACE FUNCTION f_calculo_experiencia(
  p_id          IN NUMBER,
  p_fecha_corte IN DATE DEFAULT SYSDATE
) RETURN NUMBER IS
  v_fecha    DATE;
  v_anios    NUMBER;
  v_ptje_exp NUMBER;
...
BEGIN
  -- Obtiene la fecha de contrato más antigua del postulante.
  SELECT MIN(fecha_contrato)
    INTO v_fecha
  FROM ANTECEDENTES_LABORALES
  WHERE numrun = p_id;
  
  -- Calcula los años de experiencia con base en la fecha de corte.
  v_anios := FLOOR(MONTHS_BETWEEN(p_fecha_corte, v_fecha)/12);
  
  -- Obtiene el puntaje correspondiente al rango de años de experiencia.
  SELECT t.ptje_experiencia
    INTO v_ptje_exp
  FROM PTJE_ANNOS_EXPERIENCIA t
  WHERE v_anios BETWEEN t.rango_annos_ini AND t.rango_annos_ter;
  
  RETURN ROUND(v_ptje_exp);
EXCEPTION
  WHEN NO_DATA_FOUND THEN
    prc_registra_error(p_id, 'f_calculo_experiencia', 
      'ORA-01403: No se ha encontrado ningún dato para: ' || p_id);
    RETURN 0;
  WHEN TOO_MANY_ROWS THEN
    prc_registra_error(p_id, 'f_calculo_experiencia', 
      'Múltiples rangos para años: ' || v_anios);
    RETURN 0;
  WHEN OTHERS THEN
    v_err_msg := TO_CHAR(SQLERRM);
    prc_registra_error(p_id, 'f_calculo_experiencia', 'Error general: ' || v_err_msg);
    RETURN 0;
END f_calculo_experiencia;
/

  
/* Función: f_calculo_pais
   Objetivo: Calcular el puntaje asociado al país de la institución donde postula el candidato.
   No se registran errores en esta función.
*/
CREATE OR REPLACE FUNCTION f_calculo_pais(p_id IN NUMBER) RETURN NUMBER IS
  v_cod_pais  NUMBER;
  v_ptje_pais NUMBER;
BEGIN
  -- Obtiene el código de país basado en la postulación
  SELECT i.cod_pais
    INTO v_cod_pais
  FROM POSTULACION_PASANTIA_PERFEC p
  JOIN PASANTIA_PERFECCIONAMIENTO pe ON p.cod_programa = pe.cod_programa
  JOIN INSTITUCION i ON pe.cod_inst = i.cod_inst
  WHERE p.numrun = p_id;
  
  -- Obtiene el puntaje para ese código de país
  SELECT t.ptje_pais
    INTO v_ptje_pais
  FROM PTJE_PAIS_POSTULA t
  WHERE t.cod_pais = v_cod_pais;
  
  RETURN ROUND(v_ptje_pais);
EXCEPTION
  WHEN OTHERS THEN
    RETURN 0;
END f_calculo_pais;
/
------------------------------------------------------------
-- PASO 3: Procedimiento principal: prc_procesar_postulaciones
-- Objetivo: Procesar los postulantes, calcular puntajes y almacenar el detalle.
------------------------------------------------------------
CREATE OR REPLACE PROCEDURE prc_procesar_postulaciones(
  ...
  CURSOR c_cand IS
    SELECT a.numrun,
           INITCAP(a.pnombre) || ' ' ||
           NVL(INITCAP(a.snombre) || ' ', '') ||
           INITCAP(a.apaterno) || ' ' ||
           INITCAP(a.amaterno) AS nombre_postulante
    FROM ANTECEDENTES_PERSONALES a
    WHERE a.numrun IN (SELECT numrun FROM POSTULACION_PASANTIA_PERFEC)
    ORDER BY a.numrun;
  ...
  v_err_msg    VARCHAR2(4000);
BEGIN
  -- Truncar las tablas de resultados para iniciar el proceso limpio.
  EXECUTE IMMEDIATE 'TRUNCATE TABLE DETALLE_PUNTAJE_POSTULACION';
  EXECUTE IMMEDIATE 'TRUNCATE TABLE ERROR_PROCESO';
  EXECUTE IMMEDIATE 'TRUNCATE TABLE RESULTADO_PROCESO';
  
  -- Procesa cada postulante
  FOR r IN c_cand LOOP
    v_ptje_exp   := f_calculo_experiencia(r.numrun, p_fecha_corte);
    v_ptje_pais  := f_calculo_pais(r.numrun);
    v_ptje_extra := pkg_extra.f_calculo_extra(r.numrun, p_porcentaje, v_ptje_exp, v_ptje_pais);
    
    INSERT INTO DETALLE_PUNTAJE_POSTULACION
      (run_postulante, nombre_postulante, ptje_annos_exp, ptje_pais_postula, ptje_extra)
    VALUES
      (TO_CHAR(r.numrun), r.nombre_postulante, v_ptje_exp, v_ptje_pais, v_ptje_extra);
  END LOOP;
...
EXCEPTION
  WHEN OTHERS THEN
    v_err_msg := TO_CHAR(SQLERRM);
    prc_registra_error(0, 'PA PRINCIPAL PROCESO', 'Error in prc_procesar_postulaciones: ' || v_err_msg);
    COMMIT;
END prc_procesar_postulaciones;
/
------------------------------------------------------------
-- PASO 4: Trigger para generar RESULTADO_PROCESO
-- Objetivo: Al insertar en DETALLE_PUNTAJE_POSTULACION, calcular el puntaje final
-- y actualizar o insertar en RESULTADO_PROCESO. Se omiten errores del trigger.
------------------------------------------------------------
CREATE OR REPLACE TRIGGER trg_resultado_proceso
...
BEGIN
  -- Calcular el puntaje final sumando los tres puntajes.
  ...
  BEGIN
    -- Actualiza RESULTADO_PROCESO si ya existe el registro, o lo inserta si no.
    UPDATE RESULTADO_PROCESO
       SET ptje_final_post = v_ptje_final,
           resultado_post = v_resultado
     WHERE run_postulante = :NEW.run_postulante;
     
    IF SQL%ROWCOUNT = 0 THEN
      INSERT INTO RESULTADO_PROCESO (run_postulante, ptje_final_post, resultado_post)
      VALUES (:NEW.run_postulante, v_ptje_final, v_resultado);
    END IF;
  EXCEPTION
    WHEN OTHERS THEN
      NULL;  -- Se omiten errores del trigger
  END;
END;
/
------------------------------------------------------------
...
------------------------------------------------------------
BEGIN
  prc_procesar_postulaciones(35, DATE '2024-12-31');
END;
/
------------------------------------------------------------
-- CONSULTAS DE VERIFICACIÓN (CON FORMATEO DE RUT)
------------------------------------------------------------
SELECT 
    f_formatea_rut(TO_NUMBER(run_postulante)) AS "RUN_POSTULANTE",
    nombre_postulante AS "NOMBRE_POSTULANTE",
    ptje_annos_exp AS "PTE_ANNOS_EXP",
    ptje_pais_postula AS "PTE_PAIS_POSTULA",
    ptje_extra AS "PTE_EXTRA"
FROM DETALLE_PUNTAJE_POSTULACION
ORDER BY run_postulante;

SELECT
    f_formatea_rut(TO_NUMBER(run_postulante)) AS "RUN_POSTULANTE",
    ptje_final_post AS "PTE_FINAL",
    resultado_post AS "RESULTADO_POST"
FROM RESULTADO_PROCESO
ORDER BY run_postulante;

SELECT
    numrun AS "NUMRUN",
    rutina_error AS "RUTINA_ERROR",
